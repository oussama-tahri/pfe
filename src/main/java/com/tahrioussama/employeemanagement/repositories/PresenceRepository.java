...
public interface PresenceRepository extends JpaRepository<Presence,Long> {
    ...
    long countByEmployeeAndDateBetweenAndPresent(Employee employee, LocalDate startDate, LocalDate endDate, PresenceStatus present);

    // Define a method to find presence records for a specific employee with a specific presence status
    List<Presence> findByEmployeeAndPresent(Employee employee, PresenceStatus present);
    ...
    boolean existsByEmployeeAndDateAndPresent(Employee employee, LocalDate date, PresenceStatus present);
